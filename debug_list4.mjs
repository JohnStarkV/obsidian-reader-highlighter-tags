...
// We intercept findCandidatesStripped to print the regex!
const oldFindAll2 = logic.findCandidatesStripped.bind(logic);
logic.findCandidatesStripped = function(text, snippet) {
    const escaped = snippet.trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const pattern = this.createFlexiblePattern(escaped);
    console.log("STRIPPED REGEX CREATED:");
    console.log(pattern);
    
    return oldFindAll2(text, snippet);
};
